...
let targetGroupsConfigCache: any[] = [];
let groupPrinterTypes = new Map<string, string>();
...
async function downloadMediaWithTimeout(
  ...
  message: any,
  outputFile: string,
  timeoutMs: number = 60000, // 60 segundos padrão de inatividade
  jobId?: string | null,
  ...
  return new Promise<string>((resolve, reject) => {
    const checkInterval = setInterval(() => {
      ...
      if (Date.now() - lastProgress > timeoutMs) {
        ...
        completed = true;
        console.error(`[Scraper Timeout] Download de ${path.basename(outputFile)} travou sem progresso por mais de ${timeoutMs / 1000}s.`);
        try {
      ...
      progressCallback: (downloaded, total) => {
        ...
        if (total && Number(total) > 50 * 1024 * 1024) { // Update progress only for files > 50MB
          ...
          if (percent >= lastReportedPercent + 5 || percent === 100) {
            ...
            if (jobId) activeDownloads.set(jobId, percent);
...
async function processBufferedEntry(client: TelegramClient, key: string) {
  ...
  for (let i = 0; i < photos.length; i++) {
    ...
    try {
      ...
      const tempPhotoPath = path.join(tempDir, `photo_${Date.now()}_${i}.jpg`);
  ...
  const docPhotosMap = new Map<number, string[]>(); // docMessageId -> URLs de fotos
  ...
  for (const docItem of docs) {
    ...
    try {
      ...
      tempFilePath = path.join(tempDir, `${Date.now()}_${safeFileName}`);
      ...
      if (!mediaData || typeof mediaData !== "string" || !fs.existsSync(mediaData)) {
...
async function processApprovedJob(client: TelegramClient, job: any) {
  ...
  try {
    ...
    const doc = (docMsg.media as any)?.document;
    ...
    const tags = file_name
      ...
      .split(/[_\-\s\.\,\(\)\[\]\–]+/)
      .map((t: string) => t.trim())
      .filter((t: string) => t.length > 2 && !stopWords.has(t) && /^[a-z0-9\u00C0-\u00FF]+$/i.test(t));
...
async function loadDynamicSettings(client: TelegramClient, dialogs: any[]) {
  try {
      ...
      if (groupsChanged) {
        ...
        for (const gConfig of dbGroupsConfig) {
          ...
          const foundDialog = dialogs.find(d => 
            (d.title && d.title.toLowerCase() === group.toLowerCase()) ||
...
async function startScraper() {
  ...
  if (!sessionString) {
    const savedSession = client.session.save() as any;
    console.log("\n==================================================================");
  ...
  app.post("/backfill", async (req: Request, res: Response) => {
    ...
    const { hours_back = 6, group_ids } = req.body as { hours_back?: number; group_ids?: string[] };
    ...
    setImmediate(async () => {
      ...
      try {
        ...
        for (const dialog of dialogs) {
          ...
          if (!isMonitored || !isRequested) continue;
          ...
          if (normVaultId && normDialogId === normVaultId) continue;
          ...
          try {
            ...
            const messagesInWindow = messages.filter((m: any) => {
            ...
            for (const msg of [...messagesInWindow].reverse()) {
              ...
              if (myId && senderIdStr === myId) continue;
              ...
              if (isDoc) {
                ...
                if (!isStl && !is3mf && !isCompressed) continue;
              }
  ...
  setInterval(async () => {
    ...
    if (msSinceLastEvent > UPDATE_STARVATION_MS) {
      console.warn(
        `[Watchdog] ALERTA: Nenhum evento do Telegram recebido há ${Math.round(msSinceLastEvent / 1000)}s. ` +
  ...
  setInterval(() => {
    ...
    if (diff > 5 * 60 * 1000) { // 5 minutos
      console.error(
        `[Watchdog Heartbeat] Erro crítico: Sem atualização de heartbeat há ${Math.round(diff / 1000)}s. ` +
