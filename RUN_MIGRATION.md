...
create table public.game_rewards_config (
  ...
  actions_per_reward int        not null default 10,
  credits_per_reward int        not null default 50,
...
create table public.game_daily_limits (
  id                      uuid        primary key default gen_random_uuid(),
  max_actions_per_day     int         not null default 100,
  max_credits_per_day     int         not null default 500,
  created_at              timestamptz not null default now(),
  updated_at              timestamptz not null default now()
);
...
## Passo 3: Acesse os painéis

- **Admin de Games:** http://localhost:3000/dashboard/admin/games
- **Moderação de Auditorias:** http://localhost:3000/dashboard/admin/audit-moderation

---

## ✅ O que foi adicionado ao menu admin

- **Gestão de Games** (nova seção)
  - Admin de Games
  - Moderação de Auditorias

Aparece logo após o header do painel admin!
